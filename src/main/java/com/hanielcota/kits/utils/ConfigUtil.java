...
public class ConfigUtil {
    ...
    private final Map<String, Boolean> booleanCache = new HashMap<>(); // Cache para valores boolean
    private final Map<String, Long> longCache = new HashMap<>(); // Novo cache para valores long
    private final Map<String, String> stringCache = new HashMap<>(); // Novo cache para Strings
    ...
    public boolean reload() {
        try {
            ...
            booleanCache.clear(); // Limpa o cache de boolean
    ...
    /**
     * Gets a string value from the configuration, using cache if available.
     *
     * @param path         the path to the string value in the config
     * @param defaultValue the default value if not found in config or cache
     * @return the string value
     */
    public String getStringCached(String path, String defaultValue) {
        if (stringCache.containsKey(path)) {
            return stringCache.get(path);
        }

        String value = config.getString(path, defaultValue);
        stringCache.put(path, value);
        return value;
    }
}